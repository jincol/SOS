        ...
        <div wire:loading.remove wire:target="obtenerOrdenesSalida">
            ...
            <div class="card filters-card orders-filters" x-data="{ advanced: {{ ($filtroSucursal || $filtroCompany || $filtroUsuario) ? 'true' : 'false' }} }">
                ...
                <div class="orders-filters-advanced" x-cloak x-show="advanced" x-collapse>
                    ...
                    <div class="field"><label for="salida-usuario">Usuario</label><input id="salida-usuario" class="control" type="search" wire:model.live.debounce.350ms="filtroUsuario" placeholder="Responsable de registro"></div>
                </div>
            </div>
                ...
                <div class="table-wrap">
                    <table class="data-table orders-table">
                        <thead><tr>
                            ...
                            <th>Sucursal</th><th>Bultos</th><th>Estado</th><th class="align-right">Acción</th>
                        </tr></thead>
                        <tbody>
                            @forelse($ordenesFiltradas as $orden)
                                ...
                                <tr wire:key="salida-{{ $orden['name'] }}">
                                    ...
                                    <td data-label="Fecha"><span class="cell-primary">{{ !empty($orden['date']) ? \Carbon\Carbon::parse($orden['date'])->format('d/m/Y') : 'No registrada' }}</span><span class="cell-secondary cell-clip">{{ $orden['user'] ?: 'Usuario no registrado' }}</span></td>
