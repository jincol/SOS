...
new class extends Component {
    ...
    public function cerrarComponente(){
        ...
        $this->dispatch('cerrarModalOrdenTransporte');
...
<div
    ...
    x-show="modalOpen"
    @keydown.escape.window="if (modalOpen) { modalOpen = false; $dispatch('cerrarModalOrdenTransporte') }"
>
    <div class="relative z-10" aria-labelledby="dialog-title" role="dialog" aria-modal="true">
        <!-- Background backdrop -->
        <div class="fixed inset-0 bg-gray-500/75 transition-opacity" aria-hidden="true" @click="if (modalOpen) { modalOpen = false; $dispatch('cerrarModalOrdenTransporte') }"></div>
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                <h3 class="text-lg font-semibold text-gray-900" id="dialog-title">
                                    Detalle Orden Transporte Nro: {{$ordenSeleccionada['name']}}
                                </h3>
                            ...
                            <button
                                ...
                                @click="if (modalOpen) { modalOpen = false; $dispatch('cerrarModalOrdenTransporte') }"
                        ...
                        <div class="bg-white border-gray-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
                                ...
                                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                                    <div class="p-1" x-data="{ xactiveTab: 'transporte'}">

                                        <div class="flex space-x-1 mb-6 bg-gray-100 p-1 rounded-lg">
                                            <button @click="xactiveTab='transporte'"
                                            ...
                                            <button @click="xactiveTab = 'carga'"
                                                    class="flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors duration-200 " :class="xactiveTab==='carga' ? 'tab-active' : 'tab-inactive' ">
                                                2. Carga
                                            </button>
                                        </div>
                                        ...
                                        <div x-show="xactiveTab==='transporte'" class="p-4">
                                            <div class="min-h-96">
                                                <div class="bg-gray-50 p-4 rounded-lg">
                                                    ...
                                                    <div class="space-y-2">
                                                        ...
                                                        <div class="grid grid-cols-8 gap-1">
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Tipo de Transporte</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">RUC Flete</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Razón Social</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">DNI</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Conductor</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Origen</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Destino</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Placa</div>

                                                        </div>


                                                        @foreach($ordenSeleccionada['fleet_order_transport_ids'] as $index=>$reg)
                                                            <div class="grid grid-cols-8 gap-1">
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['name'] ?: 'No registrado' }}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['freight_ruc'] }}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['freight_rs_id']}}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['dni']}}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['driver_id']}}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['origin_location_address_id']}}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['destination_location_address_id']}}</div>
                                                                <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{ $reg['transport_plate_id']}}</div>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                                        ...
                                        <div x-show="xactiveTab==='carga'" class="p-4">
                                            <div class="min-h-96 ">
                                                <div class="bg-gray-50 p-4 rounded-lg">
                                                    ...
                                                    <div class="space-y-2">
                                                        ...
                                                        <div class="grid grid-cols-7 gap-1">
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Ordenes de Salida</div>
                                                            <div class="font-medium text-gray-500 text-center rounded-sm p-2 text-xs">Tipo de Mercadería</div>
                                                        ...
                                                        @foreach($ordenSeleccionada['load_order_transport_ids'] as $index=>$reg)
                                                        <div class="grid grid-cols-7 gap-1">
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['warehouse_order_transport_id']}}</div>
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['type_merchandise']}}</div>
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['number_bundles']}}</div>
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['number_pallets']}}</div>
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['useful_weight']}}</div>
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['gross_weight']}}</div>
                                                            <div class="border-gray-200 border text-gray-900 text-center font-semibold rounded-sm p-2 text-xs">{{$reg['neto_weight']}}</div>
                                                        </div>
                                                        @endforeach
                                                    </div>
                                                </div>
